...
class PatientRegistrationForm:
    ...
    def setup_form(self):
        ...
        dob_entry = ttk.Entry(self.parent, textvariable=self.dob_var, width=30)
        dob_entry.grid(row=row, column=1, sticky=(tk.W, tk.E), pady=2, padx=(5, 0))
    ...
    def validate_phone(self, event=None):
        ...
        if len(digits_only) > 10:
            self.phone_var.set(phone[:-1])  # Remove last character
...
class PatientSearchForm:
    ...
    def setup_search_form(self):
        ...
        ).grid(row=1, column=1, sticky=tk.W, pady=5, padx=(5, 0))
    ...
    def on_patient_select(self, event):
        ...
        if selection:
            ...
            # Show patient details (could open a detail window in future)
            messagebox.showinfo(
                "Patient Selected", 
                f"Selected: {patient_name}\nID: {patient_id}\n\n(Detail view coming in next phase)"
            )